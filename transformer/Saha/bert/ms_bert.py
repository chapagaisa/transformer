...
class BERTModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        dbo = self.bert_drop(mean_last_hidden_state)
        output = self.out(dbo)
        return output
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'ms_roc_curve_bert')
...
plt.savefig('ms_bert_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('ms_bert_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('ms_bert_loss_history.png')
plt.close()