...
class MinorityStressDatSet(Dataset):
...
def eval_fn(valid_dataloader, model, device):
    ...
    save_roc_curve(fin_labels, [prob[0] for prob in fin_outputs], 'ms_roc_curve_electra')
    return fin_outputs, fin_labels, final_loss / len(valid_dataloader)


# Initialize datasets and dataloaders
train_data = MinorityStressDatSet(Config.TRAINING_DATA)
valid_data = MinorityStressDatSet(Config.VALIDATION_DATA)
test_data = MinorityStressDatSet(Config.TEST_DATA)

train_dataloader = DataLoader(train_data, batch_size=Config.TRAIN_BATCH_SIZE, num_workers=7)
...
plt.savefig('ms_electra_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('ms_electra_accuracy_history.png')
plt.close()
...
plt.legend()
plt.ylim([0, 1])
plt.savefig('ms_electra_loss_history.png')
plt.close()
